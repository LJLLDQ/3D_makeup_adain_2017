...
class VGGEncoder(nn.Module):
    ...
    def forward(self, images, output_last_feature=False):
        ...
        h4 = self.slice4(h3)
        h5 = self.slice5(h1)
        # print(h1.shape) # torch.Size([1, 64, 256, 256])
        # print(h2.shape) # torch.Size([1, 128, 128, 128])
        # print(h3.shape) # torch.Size([1, 256, 64, 64])
        # print(h4.shape) # torch.Size([1, 512, 32, 32])
        # print(self.slice5(h4).shape)
        # exit()
...
class Model(nn.Module):
    ...
    def forward(self, content_images, style_images, alpha=1.0, lam=10):
        content_features, h5 = self.vgg_encoder(content_images, output_last_feature=True)
        style_features, _, = self.vgg_encoder(style_images, output_last_feature=True)

        t = adain(content_features, style_features) # torch.Size([1, 512, 32, 32])
        # t = alpha * t + (1 - alpha) * content_features
        out = self.decoder(t) # torch.Size([1, 3, 256, 256])
        final_out = torch.cat((out, h5), dim=1)
        final_out = self.slice(final_out)
        # out = out + h5

        # loss
        output_features, _, = self.vgg_encoder(out, output_last_feature=True)
